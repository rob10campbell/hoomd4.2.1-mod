...
void export_union_faceted_ellipsoid(pybind11::module& m)
    ...
    m.def("create_IntegratorHPMCMonoFacetedEllipsoidUnion", [](std::shared_ptr<SystemDefinition> sysdef, std::shared_ptr<Variant> vinf)
    {
        return std::make_shared<IntegratorHPMCMono<ShapeUnion<ShapeFacetedEllipsoid>>>(sysdef, vinf);
    });
