...
/*! This evaluator is a variant of the Lennard-Jones pair potential, for which the force goes
   smoothly to zero at \f$ r = r_{\mathrm{cut}} \f$.
    ...
    \f[ \Delta V(r) = -(r - r_{\mathrm{cut}}) \frac{\partial V_{\mathrm{LJ}}}{\partial
   r}(r_{\mathrm{cut}}) \f]. The constant value \f$ -\frac{1}{r} \frac{\partial V}{\partial
   r}(r_{\mathrm{cut}}) \f$ is subtracted from \a force_divr .
...
class EvaluatorPairForceShiftedLJ
    ...
    */
    DEVICE EvaluatorPairForceShiftedLJ(Scalar _rsq, Scalar _radcontact, unsigned int _pair_typeids[2], Scalar _rcutsq, const param_type& _params) //~add radcontact, pair_typeIDs [PROCF2023]
