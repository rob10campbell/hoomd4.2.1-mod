...
class ConstantVolume(Thermostatted):
    ...
    Args:
        ...
        wall_axes: a string that determines the existence of walls in different
            ...
            ; defaults to "" for normal boundaries [RHEOINF]
    ...
    def __init__(self, filter, thermostat=None, wall_axes=""): ##~ add walls [REHOINF]):
        super().__init__()
        # store metadata
        param_dict = ParameterDict(filter=ParticleFilter,
                                   thermostat=OnlyTypes(Thermostat,
                                                        allow_none=True), #) ##~[RHEOINF]
    ...
    def _attach_hook(self):
        ...
        ##~ add optional wall_axes[RHEOINF]
        if self.wall_axes != "":
            self._cpp_obj.m_wall = self.wall_axes
        ##~
        super()._attach_hook()
