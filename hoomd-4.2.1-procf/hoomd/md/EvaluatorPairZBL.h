...
/*! EvaluatorPairZBL evaluates the function
    \f{eqnarray*}
        V_{\mathrm{ZBL}}(r) = & \frac{Z_i Z_j e^2}{4 \pi \epsilon_0 r_{ij}} \left[ 0.1818 \exp
   \left( -3.2 \frac{r_{ij}}{a_F} \right) + 0.5099 \exp \left( -0.9423 \frac{r_{ij}}{a_F} \right) +
   0.2802 \exp \left( -0.4029 \frac{r_{ij}}{a_F} \right) + 0.02817 \exp \left( -0.2016
   \frac{r_{ij}}{a_F} \right) \right], & r < r_{\mathrm{cut}} \\
                            = & 0, & r > r_{\mathrm{cut}} \\
    \f}
...
class EvaluatorPairZBL
    ...
    */
    DEVICE EvaluatorPairZBL(Scalar _rsq, Scalar _radcontact, unsigned int _pair_typeids[2], Scalar _rcutsq, const param_type& _params) //~ add radcontact, pair_typeIDs [PROCF2023]
        : rsq(_rsq), radcontact(_radcontact), rcutsq(_rcutsq), Zsq(_params.qi * _params.qj), aF(_params.aF) //~ add radcontact [PROCF2023]
        {
    ...
    DEVICE bool evalForceAndEnergy(Scalar& force_divr, Scalar& pair_eng, bool energy_shift)
        ...
        if (rsq < rcutsq && Zsq != 0 && aF != 0)
            ...
            force_divr += Scalar(1.0) / aF
                          * (Scalar(3.2) * exp1 + Scalar(0.9423) * exp2 + Scalar(0.4029) * exp3
                             + Scalar(0.2016) * exp4);
