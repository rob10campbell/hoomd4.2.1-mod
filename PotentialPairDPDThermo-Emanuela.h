...
template<class evaluator> class PotentialPairDPDThermo : public PotentialPair<evaluator>
    ...
    PotentialPairDPDThermo(std::shared_ptr<SystemDefinition> sysdef,
                           ...
                           Scalar w, //~ add w [RHEOINF]
                           Scalar theta_bar); //~ add theta_bar [RHEOINF]
    //! Destructor
    ...
    Scalar m_w;                  //~!< w value [RHEOINF]
    Scalar m_theta_bar;          //~!< theta_bar value [RHEOINF]
...
PotentialPairDPDThermo<evaluator>::PotentialPairDPDThermo(std::shared_ptr<SystemDefinition> sysdef,
                                                          ...
                                                          Scalar w, //~ add w [RHEOINF]
                                                          Scalar theta_bar) //~ add theta_bar [RHEOINF]
    : PotentialPair<evaluator>(sysdef, nlist), m_bond_calc(bond_calc), m_K(K), m_w(w), m_theta_bar(theta_bar), m_sysdef(sysdef) //~ add bond_calc, K, and sysdef [RHEOINF]
    {
...
template<class T> void export_PotentialPairDPDThermo(pybind11::module& m, const std::string& name)
    ...
    pybind11::class_<PotentialPairDPDThermo<T>,
        ...
        .def(pybind11::init<std::shared_ptr<SystemDefinition>, std::shared_ptr<NeighborList>, bool, Scalar, Scalar, Scalar>()) //~ add bool for bond_calc, 3 Scalars for angular repulsion params [RHEOINF]
        ...
        .def_property("w", &PotentialPairDPDThermo<T>::getW, &PotentialPairDPDThermo<T>::setW) //~ add w [RHEOINF]
        .def_property("theta_bar", &PotentialPairDPDThermo<T>::getTheta, &PotentialPairDPDThermo<T>::setTheta); //~ add theta_bar [RHEOINF]
    }
