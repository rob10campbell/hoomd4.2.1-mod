...
def msd_py(filename):
  ...
  if period >= tau_to_half:
    ...
    module.msd_calculation(data_outpath,nframes,ncolloids,allcollpos)
...
# of a simulation. 
# This code calculates g(r) for colloid-colloid interactions
#
...
def gofr_py(filename):	
  ...
  # use this to count the total number of colloids
  ncolloids = len(colloids)
  # get the typeid of all colloids from the last frame
  typeid = traj[-1].particles.typeid
  # get the xyz position of all colloids from the last frame
  pos=traj[-1].particles.position[colloids]
  # set the maximum distance considered from the center of a colloid
  ...
  module.gofr_calc(data_outpath,Lbox,m_xy,ncolloids,pos,bin_width,nlayers)
...
# choose the range of bins you want to explore (default 1 size)
