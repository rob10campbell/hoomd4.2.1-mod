...
# Morse
D0 = 7.0 * KT # attraction strength (gels at >=4kT)
...
theta_bar = 120 # angle in degrees
theta_bar_rad = theta_bar * np.pi / 180 # angle in radians
B = 0 #1 * KT #67.27 # magnitude of angular repulsion
...
if os.path.exists('Gelation-tersoff-BD.gsd'):
  if rank == 0 or comm.size == 1:
    print('Tersoff Gelation file already exists. No new files created.')
else:
  if rank == 0 or comm.size == 1:
  ...
  #tersoff = hoomd.md.many_body.Tersoff(default_r_cut=1.3, nlist=nl)
  ...
  # set output file
  gsd_writer = hoomd.write.GSD(trigger=period, filename="Gelation-tersoff-BD.gsd", 
  ...
  if rank == 0 or comm.size == 1:
    print('New Brownian Dyanmics gelation state (Gelation-tersoff-BD.gsd) created.')
