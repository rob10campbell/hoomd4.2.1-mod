...
B = 1 #1 * KT #67.27 # magnitude of angular repulsion
...
if os.path.exists('Gelation-angularrepulsion-BD.gsd'):
  if rank == 0 or comm.size == 1:
    print('Angular Repulsion Gelation file already exists. No new files created.')
else:
  if rank == 0 or comm.size == 1:
  ...
  # set output file
  gsd_writer = hoomd.write.GSD(trigger=period, filename="Gelation-angularrepulsion-BD.gsd", 
  ...
  if rank == 0 or comm.size == 1:
    print('New Brownian Dyanmics gelation state (Gelation-angularrepulsion-BD.gsd) created.')
