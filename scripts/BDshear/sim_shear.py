...
sim.timestep=0
if os.path.exists('../u%d_k%d/Gelation1.gsd'%(D0,alpha)):
    sim.create_state_from_gsd(filename='../u%d_k%d/Gelation1.gsd'%(D0,alpha))
else:
    sim.create_state_from_gsd(filename='../u%d_k%d/Gelation.gsd'%(D0,alpha))

all_ = hoomd.filter.Type(['A'])

nl = hoomd.md.nlist.Tree(buffer=0.05);
morse = hoomd.md.pair.Morse(default_r_cut=1.0, nlist=nl)
morse.params[('A', 'A')] = dict(D0=D0*kT, alpha=alpha, r0=2*R_C1)
